@echo off
title CropMind - Starting...
echo.
echo  ====================================
echo   CropMind - Farm Management System
echo  ====================================
echo.

echo  [1/3] Starting Backend API (Port 5000)...
start "CropMind Backend" cmd /k "cd /d %~dp0backend && npm start"
timeout /t 2 /nobreak >nul

echo  [2/3] Starting ML Service (Port 5001)...
where python >nul 2>&1
if %ERRORLEVEL% == 0 (
    start "CropMind ML Service" cmd /k "cd /d %~dp0ml && pip install -r requirements.txt -q && python app.py"
) else (
    where python3 >nul 2>&1
    if %ERRORLEVEL% == 0 (
        start "CropMind ML Service" cmd /k "cd /d %~dp0ml && pip3 install -r requirements.txt -q && python3 app.py"
    ) else (
...
start "CropMind Frontend" cmd /k "cd /d %~dp0frontend && npm run dev"
